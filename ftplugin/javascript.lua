...
local function add_async()
	...
	local function_node =
		find_node_ancestor({ "arrow_function", "function_declaration", "function" }, current_node)
...
-- 			local language_tree = vim.treesitter
-- 				.get_parser(bufnr)
-- 				:language_for_range({ cursor[1], cursor[2], cursor[1], cursor[2] })
--
-- 			return language_tree
-- 		end
--       vim.print(get_language_tree_for_cursor_location())
